...
async def get_current_user_id(credentials: HTTPAuthorizationCredentials = Depends(security)) -> str:
    ...
    # 从Django JWT中提取user_id（支持多种字段名）
    user_id: str = payload.get("user_id") or payload.get("sub") or payload.get("id") or payload.get("user_id_field")
    
    if user_id is None:
        # 调试：打印 payload 内容以便排查问题
        logger.info(f"【debug】JWT payload 内容: {payload}", extra={"path": "auth_utils.get_current_user_id"})
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Could not find user ID in token. Available fields: {list(payload.keys())}",
            headers={"WWW-Authenticate": "Bearer"},
        )
